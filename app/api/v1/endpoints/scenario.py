...
@base_auth_required
async def get_scenario_by_character(char: str, current_user: User = Depends(get_current_active_user), session =Depends(get_async_session)) -> Scenario | None:
    if len(char) != 1:
...
@base_auth_required
async def get_all_scenarios_as_list(current_user: User = Depends(get_current_active_user),session =Depends(get_async_session)) -> list[Scenario]:
...
@admin_auth_required
async def create_new_scenario(scenario_data: ScenarioCreate, current_user: User = Depends(get_current_active_user),session=Depends(get_async_session)) -> Scenario:
