...
from app.crud.game import create_game, get_game_by_id
...
async def post_new_game(game_init_data: GameCreate, session: AsyncSession = Depends(get_async_session)): 
...
async def get_by_id(game_id: int, session: AsyncSession = Depends(get_async_session))-> Game | None:
...
async def get_cycle_index(game_id: int, session: AsyncSession = Depends(get_async_session)) -> int | None:
