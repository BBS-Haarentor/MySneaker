...
from app.crud.cycle import get_current_cycle_by_user_id, get_cycle_by_user_id_and_index, get_last_cycle_by_user_id
from app.crud.game import get_current_cycles_by_game_id, get_current_stocks_by_game_id, get_game_by_id, get_game_state, set_back_cycle_index, turnover_next_cycle
...
async def get_my_summary(current_user: User = Depends(get_current_active_user),
    ...
    last_cycle: Cycle = await get_last_cycle_by_user_id(user_id=current_user.id, session=session)

    last_stock: Stock = await get_stock_entry_by_user_id_and_cycle_id(user_id=current_user.id, index=game.current_cycle_index-1, session=session)
    last_scenario: Scenario = await get_scenario_by_char(game.scenario_order[game.current_cycle_index-1], session=session)
    return {"current_stock": last_stock, "scenario": last_scenario, "current_cycle": last_cycle}
...
async def turnover(game_id: int,
    ...
    if not game.owner_id == current_user.id:
        ...
        if isinstance(check_admin, NoneType):
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="Insufficient credentials")
...
async def get_current_cycle(game_id: int,
                            current_user: User = Depends(get_current_active_user),
                            session: AsyncSession = Depends(get_async_session)) -> list[Stock]:
...
async def init_database(api_key: APIKey = Depends(get_api_key),
