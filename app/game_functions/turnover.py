...
async def mock_turnover(scenario: Scenario, stock_list: list[Stock], cycle_list: list[Cycle]) -> list[Stock]:
    ...
    for i in range(0, len(stock_output)):
        ...
        _sneakerkauf=cycle_list[i].buy_sneaker
        _farbenkauf=cycle_list[i].buy_paint
        ...
        _Kontostand -= round((_MitarbeiterTotal * scenario.employee_salary) * (1 + scenario.employee_cost_modfier), 2)
        logging.warning(f"{_Kontostand=}")  
