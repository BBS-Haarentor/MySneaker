...
class Company():
    ...
    def buy_new_machine(self) -> None:
        if self.cycle.buy_new_machine > 0:
            ...
            tx: Transaction = create_transaction(amount=- _machine_type.purchase_cost, company_id=self.company_id,
                                                 detail={"_machine_purchase_cost": _machine_type.purchase_cost})
            self.add_tx([tx])
        return None
    ...
    def update_research(self) -> None:
        ...
        tx: Transaction = create_transaction(amount=- actual_investment, company_id=self.company_id,
                                             detail={"actual_investment": actual_investment})
        self.add_tx([tx])
        return None
    ...
    def stock_up(self) -> None:
        ...
        tx1: Transaction = create_transaction(amount=- _purchase_cost_sneaker, company_id=self.company_id,
                                              detail={"_cost_sneaker": _purchase_cost_sneaker})
        tx2: Transaction = create_transaction(amount=- _purchase_cost_paint, company_id=self.company_id,
                                              detail={"_cost_paint": _purchase_cost_paint})
        self.add_tx([tx1, tx2])
        return None
